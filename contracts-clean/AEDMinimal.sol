...
contract AEDMinimal is UUPSUpgradeable, ERC721Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory base = "https://aed-metadata.vercel.app/api/";
        bool isBadge = isAISubdomain[tokenId];
        string memory endpoint = isBadge ? "sub" : "domain";
        
        return string(abi.encodePacked(base, endpoint, "/", _uint2str(tokenId)));
    }
    ...
    function setReverse(string calldata domain) external {
        require(owners[domainToTokenId[domain]] == msg.sender, "Not domain owner");
        
        if (bytes(reverseRecords[msg.sender]).length > 0) {
            delete reverseOwners[reverseRecords[msg.sender]];
        }
